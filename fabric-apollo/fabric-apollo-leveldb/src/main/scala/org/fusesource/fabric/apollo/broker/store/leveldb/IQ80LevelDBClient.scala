...
import org.iq80.leveldb.table._
import org.fusesource.leveldbjni.Util

/**
 * @author <a href="http://hiramchirino.com">Hiram Chirino</a>
 */
object IQ80LevelDBClient extends Log {
  ...
  final val dirty_index_key = ascii(":dirty").data
  ...
  implicit def toBufferChannel(value:Array[Byte]):ChannelBuffer = ChannelBuffers.wrappedBuffer(value)
  implicit def toByteArray(value:ChannelBuffer):Array[Byte] = {
    val rc = new Array[Byte](value.readableBytes())
    value.readBytes(rc)
    rc
  }

  final class RichDB(val db: DbImpl) {

    def getApproximateSizes(start:Array[Byte], end:Array[Byte]) = db.getApproximateSizes(start, end)

    def get(key:Array[Byte], ro:ReadOptions=new ReadOptions):Option[Array[Byte]] = {
      Option(db.get(ro, key)).map(toByteArray(_))
    }

    def delete:Unit = db.close()
    ...
    def cursor_keys(ro:ReadOptions=new ReadOptions)(func: Array[Byte] => Boolean): Unit = {
      ...
      while( iterator.hasNext && func(iterator.peek.getKey) ) {
        iterator.next()
      }
    }
    ...
    def cursor_keys_prefixed(prefix:Array[Byte], ro:ReadOptions=new ReadOptions)(func: Array[Byte] => Boolean): Unit = {
      ...
      while( iterator.hasNext && check(iterator.peek.getKey) ) {
        iterator.next()
      }
    }
    ...
    def cursor_prefixed(prefix:Array[Byte], ro:ReadOptions=new ReadOptions)(func: (Array[Byte],Array[Byte]) => Boolean): Unit = {
      ...
      iterator.seek(prefix);
      def check(key:Array[Byte]) = {
        key.startsWith(prefix) && func(key, iterator.peek.getValue)
      }
      while( iterator.hasNext && check(iterator.peek.getKey) ) {
        iterator.next()
      }
    ...
    def cursor_range(start_included:Array[Byte], end_excluded:Array[Byte], ro:ReadOptions=new ReadOptions)(func: (Array[Byte],Array[Byte]) => Boolean): Unit = {
      ...
      iterator.seek(start_included);
      def check(key:Array[Byte]) = {
        (compare(key,end_excluded) < 0) && func(key, iterator.peek.getValue)
      }
      while( iterator.hasNext && check(iterator.peek.getKey) ) {
        iterator.next()
      }
...
//      }
  ...
  def bytes(value:String) = utf8(value).toByteArray
 ...
 * @author <a href="http://hiramchirino.com">Hiram Chirino</a>
 */
class IQ80LevelDBClient(store: LevelDBStore) extends LevelDBClient {

  import IQ80LevelDBClient._
  import FileSupport._
...
//  var index_logger:Logger = _
  ...
  def start() = {
    ...
    index_options.setCreateIfMissing(true);

    config.index_max_open_files.foreach( index_options.setMaxOpenFiles(_) )
    config.index_block_restart_interval.foreach( index_options.setBlockRestartInterval(_) )
//    config.paranoid_checks.foreach( index_options.paranoidChecks(_) )
    config.index_write_buffer_size.foreach(x=> index_options.setWriteBufferSize(x.toInt) )
    config.index_block_size.foreach( index_options.setBlockSize(_) )
    Option(config.index_compression).foreach(x => index_options.setCompressionType( x match {
...
//    index_options.infoLog(index_logger)
    ...
    retry {
      ...
      index = new RichDB(new DbImpl(index_options, dirty_index_file));
      try {
...
//    index_cache = null
  ...
  def suspend() = {
    ...
    wo.setSync(true)
    index.put(dirty_index_key, bytes("false"), wo)
    index.delete
  }
  ...
  def retry[T](func: => T): T = {
    ...
    while(!rc.isDefined) {
      ...
      try {
        ...
        case e:Throwable =>
          if( error==null ) {
            warn(e, "DbImpl operation failed. (entering recovery mode)")
          }
          error = e
      }
    ...
    if( error!=null ) {
      info("DbImpl recovered from failure.")
    }
    rc.get
  }
